...
from acoustic_array.analysis import FrameStats, analyse_frame
...
class DetectedEvent:
    ...
    # Vestigial: belongs to the PARKED classroom layer, which fills it in.
    # This package never sets it, and the public AcousticEvent has no such
    # field. Kept only so heimdall.audio keeps working unchanged.
...
class AudioEventDetector:
    ...
    def classify_frame(self, frame: AudioFrame) -> tuple[EventType, float]:
        """Label one frame without run-merging. For live displays.

        Shares the classifier and the noise-floor tracker with process(), so a
        live overlay and the event log cannot disagree about what a frame is.
        """
        stats = analyse_frame(frame)
        floor = self.noise_floor.update(stats.rms)
        return self.classifier.classify(stats, floor)
